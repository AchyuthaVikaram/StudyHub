...
const NoteDetails = () => {
  ...
  if (!note) return <div className="min-h-screen flex items-center justify-center text-red-500">Note not found.</div>;
        ...
        <Card className="shadow-xl">
          <CardHeader>
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2 mb-2">
              <div className="flex items-center gap-2">
                ...
                <span className="text-slate-500 text-sm">{note.university}</span>
                {note.semester && <span className="text-slate-500 text-sm">Sem {note.semester}</span>}
              </div>
              <div className="flex items-center gap-2">
                <Star className="w-5 h-5 text-amber-500" />
                <span className="font-bold text-lg">{note.rating || 0}</span>
                <span className="text-slate-500">({note.rating_count || 0} ratings)</span>
              </div>
            </div>
            <CardTitle className="text-2xl font-bold mb-1">{note.title}</CardTitle>
            <div className="text-slate-600 mb-2">{note.description}</div>
            <div className="flex items-center gap-2 text-sm text-slate-500 mb-2">
              <User className="w-4 h-4 mr-1" />
              {note.user_profiles ? `${note.user_profiles.first_name} ${note.user_profiles.last_name}` : "Uploader"}
            </div>
          ...
          <CardContent>
            ...
            <div className="mt-8">
              ...
              {note.rating_count === 0 && <div className="text-slate-500">No ratings yet.</div>}
              {note.note_ratings && note.note_ratings.length > 0 && (
                <div className="space-y-2">
                  {note.note_ratings.map((r: any, idx: number) => (
                    <div key={idx} className="flex items-center gap-2 text-sm text-slate-700 border-b py-2">
                      <Star className="w-4 h-4 text-amber-500" />
                      <span>{r.rating}</span>
                      <span className="text-slate-500 ml-2">by {r.user_id === (user && user.id) ? "You" : r.user_id}</span>
                    </div>
                  ))}
                </div>
              )}
