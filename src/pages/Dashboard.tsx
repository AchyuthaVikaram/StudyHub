          ...
          <CardContent>
            {editMode ? (
              ...
              <div className="space-y-2">
                ...
                <div><b>Semester:</b> {profile.semester}</div>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="bg-blue-100 rounded p-4 text-center">
                <div className="text-2xl font-bold">{dashboard?.stats.uploadedNotes}</div>
                <div>Notes Uploaded</div>
              </div>
              <div className="bg-purple-100 rounded p-4 text-center">
                <div className="text-2xl font-bold">{dashboard?.stats.totalDownloads}</div>
                <div>Total Downloads</div>
              </div>
              <div className="bg-yellow-100 rounded p-4 text-center">
                <div className="text-2xl font-bold">{dashboard?.stats.ratingsGiven}</div>
            ...
            {dashboard?.recentUploads?.length === 0 ? (
              ...
              <div className="space-y-2">
                {dashboard?.recentUploads?.map((note: any) => (
                  <div key={note.id} className="border rounded p-2 flex flex-col md:flex-row md:items-center md:justify-between">
                    <div>
                      <b>{note.title}</b> <span className="text-slate-500">({note.subject})</span>
                    </div>
                    <div className="text-sm text-slate-500">{note.upload_date?.slice(0,10)} | Downloads: {note.downloads} | Rating: {note.rating}</div>
                  </div>
                ))}
              </div>
            )}
